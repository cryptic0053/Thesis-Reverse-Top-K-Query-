...
OUT_CSV   = os.path.join(BASE_DIR, "outputs", "csv",   "corrected_movielens_temporal")
...
def main():
    print("=" * 64)
    print("  Corrected MovieLens Temporal Evaluation")
    print("=" * 64)
    ...
    methods = [
        "Full_SSA", "Full_PRA",
        "Hybrid_SSA_GlobalAvg", "Hybrid_PRA_GlobalAvg",
        "Hybrid_SSA_UnionWin", "Hybrid_PRA_UnionWin",
    ...
    for qi, q_idx in enumerate(queries):
        ...
        hybrid_calls = [
            ("Hybrid_SSA_GlobalAvg", lambda: hybrid_ssa_query(S, W, q_idx, K, C, tb, te, TAU, T_global, THR_global)),
            ("Hybrid_PRA_GlobalAvg", lambda: hybrid_pra_query(S, W, q_idx, K, C, tb, te, TAU, T_global, THR_global)),
            ("Hybrid_SSA_UnionWin", lambda: hybrid_ssa_query_union(S, W, q_idx, K, C, tb, te, TAU, T_table_list, THR_list)),
            ("Hybrid_PRA_UnionWin", lambda: hybrid_pra_query_union(S, W, q_idx, K, C, tb, te, TAU, T_table_list, THR_list)),
    ...
    # Calculate aggregates for non-empty baselines
    ...
    if num_non_empty > 0:
        ...
        recalls = [s["Recall"] for s in summary]
        plt.barh(names, recalls, color='skyblue')
        plt.title('Average Recall (Corrected Larger-is-Better Convention)')
        plt.xlabel('Recall')
        plt.tight_layout()
        plt.savefig(os.path.join(OUT_PLOTS, 'corrected_recall_bar.png'))
        plt.close()
